...
- Local worktree: Beta 0.8.15 vocabulary accuracy and compact-interaction implementation, signed release artifacts, installed-Android verification, and CI verification complete
- Latest confirmed GitHub Actions state: CI run `29547431783` passed build, public-isolation/content/documentation gates, learning modules, browser motion and maturity regressions, phrase-aware vocabulary study QA, Fly View collection QA, and Android-debug packaging for implementation commit `bbfcce68f5aa5b79a057e8a753fe423cb1e12b19`
...
- Implementation commit `bbfcce68f5aa5b79a057e8a753fe423cb1e12b19` passed GitHub Actions run `29547431783`: <https://github.com/Felix-Zuo/six-sigma-study-app/actions/runs/29547431783>.
...
- Implementation commit `25cf763a58aa897308be1ca4962368d0ea8114d9` passed GitHub Actions run `29543807607`: <https://github.com/Felix-Zuo/six-sigma-study-app/actions/runs/29543807607>.
...
- Feature commit `0370efcf31c94b623e760cf792570331c10b14b7` passed GitHub Actions run `29515394327`. Final implementation commit `c69a83d749f9dc01eda5df4eef296d563c986e43`, which adds an explicit image-decode wait to remove a startup-mark QA race, passed run `29515804792`: <https://github.com/Felix-Zuo/six-sigma-study-app/actions/runs/29515804792>.
- Final local release artifacts:
...
- Researched the public 不背单词 product surface and separated observable experience from undisclosed scheduling internals. The implementation adopts its concise dictionary/example-led recall flow without claiming to reproduce its private algorithm.
- Replaced the legacy interval calculation with maintained `ts-fsrs` scheduling at a 90% desired retention. Existing records migrate lazily and retain book, source, streak, context, and AI metadata; bounded review history supports future diagnostics.
- Review now starts with direct dictionary-meaning retrieval. `忘记`, `模糊`, and `记得` map to FSRS Again, Hard, and Good, and each option previews the resulting interval before commitment.
- Forgotten and fuzzy cards return at the end of the same session for short reinforcement. These repeats do not inflate daily completion or apply a second long-term reschedule; forgotten cards are capped at two reinforcement appearances per session.
- Kept the answer surface compact: canonical meaning and a short bilingual source example remain primary, the exact target occurrence is underlined, and context, morphology, source, and optional AI assistance use progressive disclosure. Every new card resets only its internal scroll.
- Strengthened phrase-aware distractors and async dictionary readiness. Distractors are same-kind where possible and bounded in length; a fallback card upgrades in place once the dictionary arrives instead of retaining stale choices.
- Added deterministic scheduler/unit QA plus mobile CDP coverage for migration, direct recall, phrase answers, interval previews, short examples, reinforcement, summary, fixed rating controls, AI persistence, and zero overflow. Learning, lexical, multi-book, maturity, AI-context, motion, transition, and Target 4 suites remain green.
- Android 16 native verification passed with a unified Back hierarchy for active route transitions, question sessions, Reader tools, immersive mode, and lookup sheets. The Capacitor listener remains the production path; target-36 currently declares the official temporary predictive-back compatibility opt-out, while API 36 automated CDP uses a document `backbutton` bridge because synthetic ADB `KEYCODE_BACK` is not delivered to target-36 apps.
- Installed package metadata: `versionCode 812`, `versionName 0.8.12-beta`. Native lookup-sheet chrome/body containment, AI entry points, chapter completion, and Back priority passed on `emulator-5554`.
- GitHub Actions run `29476433910` passed all build, browser-QA, vocabulary, maturity, and Android-debug jobs for release commit `4ea4f6c`.
- Final local release artifacts:
  - APK: 41,272,431 bytes; SHA-256 `3A1915C3A3FCC5F8AD76C809D9E3262918FD10C9A8EDC71165AE5C2111E2865E`; APK Signature Scheme v2 verified with one signer.
  - AAB: 39,023,620 bytes; SHA-256 `B69688BADE9418558D425B10AFD4C4E17A557F0907C0576EA4E146E95C5C2684`; JAR signature verification passed with expected local self-signed/no-timestamp warnings.
