...
async function main() {
  ...
  await waitFor("phrase prompt", () => evaluate(`document.querySelector(".flashCard h2")?.textContent?.trim() === "trial and error"`));
  await evaluate(`Array.from(document.querySelectorAll(".flashPromptActions button")).find((item) => item.textContent.includes("想起来了"))?.click()`);
  await waitFor("dictionary quiz", () => evaluate(`Boolean(document.querySelector(".flashQuiz"))`));
  const phraseQuiz = await evaluate(`(() => ({
    prompt: document.querySelector(".flashQuiz p")?.textContent?.trim(),
    options: Array.from(document.querySelectorAll(".flashQuiz button")).map((item) => item.textContent.trim())
  }))()`);
  ...
  await evaluate(`Array.from(document.querySelectorAll(".flashRatingActions button")).find((item) => item.textContent.trim() === "认识")?.click()`);
  await waitFor("throughout prompt", () => evaluate(`document.querySelector(".flashCard h2")?.textContent?.trim() === "throughout"`));
  await evaluate(`Array.from(document.querySelectorAll(".flashPromptActions button")).find((item) => item.textContent.includes("想起来了"))?.click()`);
  await waitFor("throughout quiz", () => evaluate(`Boolean(document.querySelector(".flashQuiz"))`));
  const throughoutQuiz = await evaluate(`Array.from(document.querySelectorAll(".flashQuiz button")).map((item) => item.textContent.trim())`);
  await evaluate(`Array.from(document.querySelectorAll(".flashQuiz button")).find((item) => item.textContent.includes("自始至终"))?.click()`);
  ...
  const checks = {
    ...
    dictionaryQuizCopy: phraseQuiz.prompt === "它的常用词典释义是：",
    phraseDictionaryOption: phraseQuiz.options.includes("反复试验") && !phraseQuiz.options.some((item) => item.includes("审判")),
    phraseAnswer: phraseAnswer.dictionary === "反复试验" && phraseAnswer.underlined === "trial and error" && phraseAnswer.exampleLength <= 170,
    ...
    contextNotQuiz: throughoutQuiz.some((item) => item.includes("自始至终")) && !throughoutQuiz.includes("后续"),
    throughoutDictionary: throughoutAnswer.dictionary?.includes("自始至终") && throughoutAnswer.underlined === "throughout",
    ...
    throughoutDockAndLayout: throughoutAnswer.dockVisible && throughoutAnswer.horizontalOverflow <= 1
  };
  const ok = Object.values(checks).every(Boolean);
  cdp.close();
  console.log(JSON.stringify({ ok, checks, migrated, staleSource, phraseQuiz, phraseAnswer, throughoutQuiz, throughoutAnswer, screenshots: { phraseShot, throughoutShot } }, null, 2));
